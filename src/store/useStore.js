...
const persisted = readPersistedSync('hulu-state');
...
const useStore = create((set, get) => ({
  ...
  transactions: persisted.transactions ?? [
    { id: 1, title: 'Ethiopian Electric Utility', subtitle: 'Electricity Bill', amount: -450.00, type: 'bill', time: '2 min ago', refId: 'HLU-TX-928471' },
    { id: 2, title: 'Salary Deposit', subtitle: 'Abyssinia Bank', amount: 32500.00, type: 'income', time: '1 hr ago', refId: 'HLU-TX-928470' },
    { id: 3, title: 'Water & Sewerage', subtitle: 'Utility Payment', amount: -125.00, type: 'bill', time: '3 hrs ago', refId: 'HLU-TX-928469' },
    { id: 4, title: 'Ethio Telecom', subtitle: 'Mobile Recharge', amount: -200.00, type: 'bill', time: 'Yesterday', refId: 'HLU-TX-928468' },
    { id: 5, title: 'Transfer to Abebe', subtitle: 'Personal Transfer', amount: -5000.00, type: 'transfer', time: 'Yesterday', refId: 'HLU-TX-928467' },
  ],
  ...
  addTransaction: (tx) => {
    const newTx = {
      ...
      refId: tx.refId || `HLU-TX-${Date.now().toString().slice(-6)}`,
  ...
  notifications: [
    ...
    { id: 5, type: 'system', title: 'System Update', titleAm: 'የሲስተም ዝማኔ', desc: 'HULU v1.1 is now available with enhanced AI Legal features', descAm: 'HULU v1.1 በተሻሻለ AI Legal ባህሪ ተገኝቷል', time: 'Yesterday', read: true },
  ],
...
async function _persistEncrypted(partial) {
  try {
    const existing = await readPersistedAsync('hulu-state');
    await writePersisted({ ...existing, ...partial }, 'hulu-state');
  } catch { /* noop */ }
}
...
(async () => {
  try {
    const data = await readPersistedAsync('hulu-state');
